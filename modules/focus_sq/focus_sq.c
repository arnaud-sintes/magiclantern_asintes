...
static struct fsq_data_t g_data = {
    .constants = {
        .max_focus_positions = 1024,
        .highlight_duration_ms = 1000,
        .infinite_focus_distance_trigger = 65535,
        .overlay_position_x = 0,  // sticked to left
        .overlay_position_y = 38, // offset from LV screen top
        .settings_file = "ML/SETTINGS/focus_sq.cfg",
        .asap_step_number = 100,
        .focus_point_increment_s = 0.1,
    },
    .task_running = false,
    .screen_on = true,
    .index = 0,
...
void fsq_print_bmp(char *_fmt, ...)
{
    ...
    int x = g_data.constants.overlay_position_x;
    int y = g_data.constants.overlay_position_y;
...
char *fsq_print_overlay__edit_play_content(size_t _cycle)
{
    ...
    {
        static char *const p_transition[6] = {"==>", " ==", "  =", "   ", ">  ", "=> "};
        snprintf(step, FSQ_CONTENT_LENGTH, "%s (% 2d)", p_transition[_cycle % 6], g_data.display.sequence_index);
    }
    else
    {
        size_t pos = (g_data.display.sequence_edited_timepoint_ms != 0) ? _cycle % 2 : 0;
    ...
    bool position_is_different = (g_data.display.state == fsq_edit) && (g_data.display.focus_distance_cm != lens_info.focus_dist);
    size_t pos = 3;
    if (g_data.display.transition_target_ms == 0)
    {
    ...
    p_edit_left = g_data.display.delimiters[0][pos];
    p_edit_right = g_data.display.delimiters[1][pos];
    unsigned focus_distance_cm = ((g_data.display.transition_target_ms != 0) || position_is_different)
                                     ? lens_info.focus_dist
                                     : g_data.display.focus_distance_cm;
    static char focus_distance_buffer[FSQ_CONTENT_LENGTH + 1];
    if (focus_distance_cm == g_data.constants.infinite_focus_distance_trigger)
    {
    ...
    else
    {
        ...
        else
        {
            ...
            double focus_distance_m = (double)focus_distance_cm / (double)100;
    ...
    // duration part:
    int current_clock_ms = get_ms_clock();
    if (g_data.display.transition_target_ms != 0)
    {
    ...
    else
    {
        ...
        if (g_data.display.transition_target_ms != 0)
        {
            duration_s = ((double)(g_data.display.transition_target_ms - current_clock_ms)) / (double)1000;
        }
...
char *fsq_print_overlay__content(size_t _cycle)
{
    ...
    // deal with messages:
    static char *const p_messages[2] = {"checking lens limits", "calibrating lens"};
...
void fsq_print_overlay(size_t _cycle)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        if (*p_edit_timepoint != 0 && (current_clock_ms - *p_edit_timepoint) > g_data.constants.highlight_duration_ms)
        {
            *p_edit_timepoint = 0;
        }
    }
    ...
    // header:
    static char *p_headers[4] = {"    ", "edit", "play", "----"};
    static char *const p_header_progress[6] = {" .oO", ".oOo", "oOo.", "Oo. ", "o. .", ". .o"};
    p_headers[3] = p_header_progress[_cycle % 6];
    size_t header_index = (g_data.display.state < 3) ? g_data.display.state : 3;

    // print:
    size_t pos = (g_data.display.state_edited_timepoint_ms != 0) ? (_cycle % 2) << 2 : 4;
    char *const p_edit_left = g_data.display.delimiters[0][pos];
    char *const p_edit_right = g_data.display.delimiters[1][pos];
...
{
    FIO_RemoveFile(g_data.constants.settings_file);
    FILE *p_file = FIO_CreateFile(g_data.constants.settings_file);
...
{
    FILE *p_file = FIO_OpenFile(g_data.constants.settings_file, O_RDONLY);
    if (p_file == 0)
    {
        return;
    }
...
void fsq_set_focus_position_normalizer()
{
    ...
    while (lens_focus_ex(g_data.constants.asap_step_number, 3, true, true, 0))
    {
    }

    // store last focus position:
    int last_focus_position = lens_info.focus_pos;

    // go to the very first position asap:
    while (lens_focus_ex(g_data.constants.asap_step_number, 3, false, true, 0))
    {
    }
...
size_t fsq_closest_step_from_position(unsigned _focus_position)
{
    ...
    {
        unsigned focus_position = *((unsigned *)vector_get(&g_data.store.focus_positions, step));
...
void fsq_dump_focus_positions()
{
    ...
    size_t step_0 = 0;
    size_t step_1 = step_range - 1;
    fsq_dump_focus_position_range(step_0, step_1);

    // dump last focus positions:
    size_t step_3 = g_data.store.focus_positions.count - 1;
    size_t step_2 = g_data.store.focus_positions.count - step_range;
...
void fsq_evaluate_step_size(size_t _mode)
{
    ...
    size_t step_0 = fsq_closest_step_from_position(fsq_normalized_focus_position());

    // move lens, 8 iterations to average the output values:
    size_t loop = 8;
    unsigned step_size = (_mode == fsq_mode_3) ? 3 : 2;
...
void fsq_evaluate_step_size_speed(bool _forward, size_t _mode)
{
    ...
    size_t current_step = fsq_closest_step_from_position(fsq_normalized_focus_position());
    ...
    fsq_log("{fsq} step size %d: %s steps per second\n", step_size, format_float_ex(step_size_speed, 3, speed_buffer, FSQ_CONTENT_LENGTH));
...
{
    int step_1 = (int)fsq_closest_step_from_position(_focus_position_1);
    int step_2 = (int)fsq_closest_step_from_position(_focus_position_2);
    int range = step_2 - step_1;
    if (_p_range != NULL)
    {
        *_p_range = range;
    }
...
struct fsq_distribution_t fsq_distribute_modes(size_t _step_range, double _target_duration_s)
{
    ...
    // slowest:
    else if (durations[2] < _target_duration_s)
    {
    ...
    else
    {
        ...
        double ratio = 1 - ((_target_duration_s - durations[offset]) / durations[offset + 1]);
        ...
        for (double delta = -0.2; delta < (double)0.2; delta += (double)0.005)
        {
            ...
            double current_distribution_duration = ABS(_target_duration_s -
...
void fsq_go_to()
{
    ...
    double real_duration_s = (double)(get_ms_clock() - t_before_ms) / (double)1000.0;

    // wait for current stabilized focus position:
    wait_for_stabilized_focus_position();

    // get target and real focus step values:
    int target_step = (int)fsq_closest_step_from_position(p_focus_point->normalized_position);
    unsigned real_focus_position = fsq_normalized_focus_position();
    int real_step = (int)fsq_closest_step_from_position(real_focus_position);
    ...
    if (real_step != target_step)
    {
        ...
        int range = target_step - real_step;
    ...
    int deviation_after = (int)p_focus_point->normalized_position - (int)corrected_real_focus_position;

    // dump deviation without correction:
    static char deviation_buffer[FSQ_CONTENT_LENGTH + 1];
    char *const p_duration_deviation = format_float_ex(real_duration_s - p_focus_point->distribution.duration_s,
    ...
    {
        int deviation_before = (int)p_focus_point->normalized_position - (int)real_focus_position;
...
void fsq_go_to_asap()
{
    ...
    unsigned current_focus_position = fsq_normalized_focus_position();
    ...
    int range = 0;
    struct fsq_distribution_t distribution = fsq_compute_distribution_between(current_focus_position,
    ...
    int target_step = (int)fsq_closest_step_from_position(p_focus_point->normalized_position);
    int real_step = (int)fsq_closest_step_from_position(fsq_normalized_focus_position());
...
void fsq_action__edit__add_focus_point()
{
    ...
    if (g_data.store.focus_points.count > 0)
    {
        ...
        focus_point.duration_s = (double)((int)((focus_point.distribution.duration_s +
                                                 g_data.constants.focus_point_increment_s) *
                                                10)) /
                                 10;
...
void fsq_action__edit__update_transition_speed(bool _increase)
{
    ...
    p_focus_point->duration_s += g_data.constants.focus_point_increment_s * (_increase ? 1 : -1);
...
unsigned int fsq_key_handler(unsigned int _key)
{
    ...
    void *const p_function = functions[g_data.display.state - 1][_key];
    if (p_function != NULL)
    {
        ((void (*)())p_function)();
        return 0;
