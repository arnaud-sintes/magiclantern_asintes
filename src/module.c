...
char *core_modules[] = {"mlv_lite", "crop_rec", "mlv_play", "mlv_snd", "sd_uhs", "lua", "file_man", "dual_iso", "silent", "ettr", "focus_sq"};
    ...
    if(!module_autoload_disabled)
    {
        ...
        else
        {
            ...
            if(!first_run)
            {
                ...
                char preset_dir6[0x80];
                char preset_dir7[0x80];
                char preset_dir8[0x80];
                ...
                snprintf(preset_dir6, sizeof(preset_dir6), "ML/SETTINGS/CM6", get_config_dir());
                snprintf(preset_dir7, sizeof(preset_dir7), "ML/SETTINGS/CM7", get_config_dir());
                snprintf(preset_dir8, sizeof(preset_dir8), "ML/SETTINGS/CM8", get_config_dir());
                ...
                if (!is_dir(preset_dir6)) { FIO_CreateDirectory(preset_dir6); }
                if (!is_dir(preset_dir7)) { FIO_CreateDirectory(preset_dir7); }
                if (!is_dir(preset_dir8)) { FIO_CreateDirectory(preset_dir8); }
                config_load();
                config_preset_scan();
            }
